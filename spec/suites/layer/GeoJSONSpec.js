...
describe("Polyline (multi) #toGeoJSON", () => {
  it("returns a 2D MultiLineString object", () => {
    const multiPolyline = new Polyline([
      [
        [10, 20],
        [2, 5],
      ],
            ...
            geometries: [
              {
                type: "LineString",
                coordinates: [
                  [-122.442559, 37.806664],
                  [-122.442838, 37.806636],
                ],
              },
              {
                type: "LineString",
                coordinates: [
                  [-122.442551, 37.806626],
                  [-122.442834, 37.8066],
                ],
              },
            ],
...
describe("GeoJSON functions", () => {
    ...
    it("creates a Layer using coordsToLatLng option (MultiPoint)", () => {
      ...
      expect(
        layer
          .getLayers()[0]
          .getLatLng()
          .equals(new LatLng(3, 2, 4)),
      ).to.be.true;
      expect(
        layer
          .getLayers()[1]
          .getLatLng()
          .equals(new LatLng(6, 5, 7)),
      ).to.be.true;
    });
  });
  ...
  describe("#latLngToCoords", () => {
    ...
    it("returns an array of coordinates with given precision", () => {
      const coords = GeoJSON.latLngToCoords(new LatLng(2.123456, 1.123456), 3);
      const coordsWithAlt = GeoJSON.latLngToCoords(new LatLng(2.123456, 1.123456, 3.123456), 3);
  ...
  describe("#latLngsToCoords", () => {
    it("accepts multidimensional latlng array", () => {
      const coords = GeoJSON.latLngsToCoords([
        [2, 1, 3],
        [5, 4, 6],
      ]);
