...
describe("Popup", () => {
  ...
  it("closes on map click when map has closePopupOnClick option", () => {
    ...
    const popup = new Popup().setLatLng(center).openOn(map);
  ...
  it("closes on map click when popup has closeOnClick option", () => {
    ...
    const popup = new Popup({ closeOnClick: true }).setLatLng(center).openOn(map);
  ...
  it("does not close on map click when popup has closeOnClick: false option", () => {
    ...
    const popup = new Popup({ closeOnClick: false }).setLatLng(center).openOn(map);
  ...
  it("sets a custom 'closeButtonLabel' on the close button", () => {
    ...
    const popup = new Popup({ closeButtonLabel }).setLatLng(center).openOn(map);

    expect(popup.getElement().querySelector(`[aria-label="${closeButtonLabel}"]`)).not.to.equal(
      null,
    );
  });
  ...
  it("prevents an underlying map click for Layer", () => {
    ...
    map.on("click", (e) => {
      mapClicked = true;
      new Popup().setLatLng(e.latlng).setContent("map popup").openOn(map);
    });
  ...
  it("opens popup with passed latlng position while initializing", () => {
    const popup = new Popup(center).openOn(map);
    expect(map.hasLayer(popup)).to.be.true;
  });

  it("opens popup with passed latlng and options position while initializing", () => {
    const popup = new Popup(center, { className: "testClass" }).addTo(map);
    expect(map.hasLayer(popup)).to.be.true;
  ...
  it("adds popup with passed content in options while initializing", () => {
    const popup = new Popup(center, { content: "Test" }).addTo(map);
    expect(map.hasLayer(popup)).to.be.true;
  ...
  describe("Map#openPopup", () => {
    ...
    it("should not be closen when dragging map", (done) => {
      ...
      mouse
        .moveTo(coords.left + 100, coords.left + 100, 0)
        .down()
        .moveBy(10, 10, 20)
        .up();
    });
    ...
    it("moves the map over a short distance to the popup if it is not in the view (keepInView)", (done) => {
      ...
      const p = new Popup({ keepInView: true })
        .setContent("Popup")
        .setLatLng(map.getBounds()._northEast);
      map.once("moveend", () => {
    ...
    it("moves the map over a long distance to the popup if it is not in the view (keepInView)", (done) => {
      ...
      const p = new Popup({ keepInView: true })
        .setContent("Popup")
        .setLatLng([center[0], center[1] + 50]);
      map.once("moveend", () => {
        expect(spy.callCount).to.equal(1);
        expect(map.getBounds().contains(p.getLatLng())).to.be.true;
        done();
  ...
  describe("Layer#_popup", () => {
    ...
    it("does not close popup when clicking on it's tip", () => {
      ...
      container.style.top = "0";
      container.style.left = "0";
      const popup = new Popup().setLatLng(map.getCenter()).openOn(map);
    ...
    it("does not open for empty FeatureGroup", () => {
      const popup = new Popup();
      new FeatureGroup([]).addTo(map).bindPopup(popup).openPopup();
    ...
    it("uses only visible layers of FeatureGroup for popup content source", () => {
      ...
      const popup = new Popup();
      const group = new FeatureGroup([marker1, marker2, marker3]).bindPopup(popup).addTo(map);
