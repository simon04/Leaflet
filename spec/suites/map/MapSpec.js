...
describe("Map", () => {
  ...
  describe("#remove", () => {
    ...
    describe("corner case checking", () => {
      it("throws an exception upon reinitialization", () => {
        expect(() => new LeafletMap(container)).to.throw("Map container is already initialized.");
      });

      it("throws an exception if a container is not found", () => {
        expect(() => new LeafletMap("nonexistentdivelement")).to.throw("Map container not found.");
      });
    });
  ...
  describe("#getBoundsZoom", () => {
    ...
    it("returns multiples of zoomSnap when zoomSnap > 0", () => {
      ...
      expect(map.getBoundsZoom(bounds, false, padding)).to.be.within(19.686456, 19.6864561);
    });
  ...
  describe("#getPixelWorldBounds", () => {
    ...
    it("return changed map bounds if really zoomed in", () => {
      ...
      expect(map.getPixelWorldBounds()).to.eql(
        new Bounds([1.4901161193847656e-8, 1.4901161193847656e-8], [268435456, 268435456]),
      );
    });
  ...
  describe("#DOM events", () => {
    ...
    it("prints to console.error about deprecated mouse* events", () => {
      ...
      expect(errorSpy.firstCall.args[0]).to.eq(
        "The event mouseover has been removed. Use the PointerEvent variant instead.",
      );
    });
  });
  ...
  describe("#Geolocation", () => {
    it("doesn't throw error if location is found and map is not existing", () => {
      ...
      expect(() => {
        fn({
          coords: { latitude: 40.415296, longitude: 10.7419264, accuracy: 1129.5646101470752 },
        });
      }).to.not.throw();
    });
  ...
  describe("#panInsideBounds", () => {
    ...
    it("pans to closest view in bounds", () => {
      ...
      const expectedCenter = new LatLng([41.59452223189, -74.273864746]);
  ...
  describe("#project", () => {
    ...
    it("returns pixel coordinates relative to the top-left of the CRS extents", () => {
      ...
      expect(a.y).to.be.approximately(3101.32046, tolerance);
    });
    ...
    it("test the prev coordinates with different zoom", () => {
      ...
      expect(b.y).to.be.approximately(6202.64092, tolerance);
    });
  });
