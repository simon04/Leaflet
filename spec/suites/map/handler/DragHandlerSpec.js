...
describe("DragHandler", () => {
  ...
  describe("#addHook", () => {
    ...
    it(
      "calls the map with dragging disabled and worldCopyJump enabled; " +
  ...
  describe("pointer events", () => {
    it("change the center of the map", (done) => {
      ...
      pointer
        .moveTo(start.x, start.y, 0)
        .down()
        .moveBy(5, 0, 20)
        .moveTo(finish.x, finish.y, 1000)
        .up();
    });
    ...
    describe("in CSS scaled container", () => {
      ...
      it("change the center of the map, compensating for CSS scale", (done) => {
        ...
        pointer
          .moveTo(startScaled.x, startScaled.y, 0)
          .down()
          .moveBy(5, 0, 20)
          .moveTo(finishScaled.x, finishScaled.y, 1000)
          .up();
      });
    });
    ...
    it("does not change the center of the map when pointer is moved less than the drag threshold", (done) => {
      ...
      pointer.moveTo(200, 200, 0).down().moveBy(1, 0, 20).moveBy(1, 0, 200).up();
    });
    ...
    it("does not trigger preclick nor click", (done) => {
      ...
      pointer.moveTo(200, 200, 0).down().moveBy(5, 0, 20).moveTo(456, 232, 200).up();
    });
    ...
    it("does not trigger preclick nor click when dragging on top of a static marker", (done) => {
      ...
      // Draggable.
      pointer.moveTo(300, 280, 0).down().moveBy(5, 0, 20).moveBy(20, 20, 100).up();
    });
    ...
    it("does not trigger preclick nor click when dragging a marker", (done) => {
      ...
      // We move 5 pixels first to overcome the 3-pixel threshold of
      // Draggable.
      pointer.moveTo(300, 280, 0).down().moveBy(5, 0, 20).moveBy(50, 50, 100).up();
    });
  ...
  describe("touch events", () => {
    ...
    it.skipIfNotTouch(
      "does not change the center of the map when finger is moved less than the drag threshold",
      (done) => {
        map = new LeafletMap(container, {
          dragging: true,
          inertia: false,
        ...
        // We move 2 pixels to stay below the default 3-pixel threshold of
        // Draggable. This should result in a click and not a drag.
        toucher.moveTo(200, 200, 0).down().moveBy(1, 0, 20).moveBy(1, 0, 200).up();
      },
    );
    ...
    it.skipIfNotTouch("reset itself after pointerup", (done) => {
      ...
      hand.sync(5);
      f1.moveTo(275, 300, 0).down().moveBy(-200, 0, 1000).up();
      // This finger should touch me map after the other one.
      f2.wait(10).moveTo(325, 300, 0).down().moveBy(210, 0, 1000).up();
    });
  });
});
