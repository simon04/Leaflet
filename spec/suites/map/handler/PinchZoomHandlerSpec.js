...
describe("PinchZoomHandler", () => {
  ...
  it.skipIfNotTouch("Increases zoom when pinching out", (done) => {
    ...
    hand.sync(5);
    f1.wait(100).moveTo(275, 300, 0).down().moveBy(-200, 0, 500).up(100);
    f2.wait(100).moveTo(325, 300, 0).down().moveBy(200, 0, 500).up(100);
  });
  ...
  it.skipIfNotTouch("Decreases zoom when pinching in", (done) => {
    ...
    hand.sync(5);
    f1.wait(100).moveTo(75, 300, 0).down().moveBy(200, 0, 500).up(100);
    f2.wait(100).moveTo(525, 300, 0).down().moveBy(-200, 0, 500).up(100);
  });
  ...
  it.skipIfNotTouch("Dragging is possible after pinch zoom", (done) => {
    ...
    hand.sync(5);
    f1.wait(100).moveTo(75, 300, 0).down().moveBy(200, 0, 500).up();
  ...
  it.skipIfNotTouch(
    "Layer is rendered correctly while pinch zoom when zoomAnim is true",
    (done) => {
      map.remove();
      ...
      hand.sync(5);
      f1.wait(100).moveTo(75, 300, 0).down().moveBy(200, 0, 500).up();
      f2.wait(100).moveTo(525, 300, 0).down().moveBy(-200, 0, 500).up();
    },
  );
  ...
  it.skipIfNotTouch("disables pinchZoom when touchZoom is false (backward compatibility)", () => {
    ...
    expect(warnSpy.firstCall.args[0]).to.eq(
  ...
  it.skipIfNotTouch(
    "enables pinchZoom when touchZoom is true and pinchZoom is false (touchZoom takes precedence)",
    () => {
      const warnSpy = sinon.spy(console, "warn");
