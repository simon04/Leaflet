...
describe("AttributionControl", () => {
  ...
  describe("#addAttribution", () => {
    ...
    it("adds several attributions listed with comma", () => {
      ...
      expect(controlContainer.innerHTML).to.eql(
        'prefix <span aria-hidden="true">|</span> foo, bar',
      );
    });
  });
  ...
  describe("on layer add/remove", () => {
    it("changes text", () => {
      ...
      fooLayer.getAttribution = function () {
        return "foo";
      };
      barLayer.getAttribution = function () {
        return "bar";
      };
      bazLayer.getAttribution = function () {
        return "baz";
      };
      ...
      expect(controlContainer.innerHTML).to.eql(
        'prefix <span aria-hidden="true">|</span> foo, bar, baz',
      );

      map.removeLayer(fooLayer);
      expect(controlContainer.innerHTML).to.eql(
        'prefix <span aria-hidden="true">|</span> bar, baz',
      );
    ...
    it("keeps count of duplicated attributions", () => {
      ...
      fo2Layer.getAttribution = function () {
        return "foo";
      };
      fo3Layer.getAttribution = function () {
        return "foo";
      };
