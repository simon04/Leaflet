...
const getIntegrity = (path) =>
  new Promise((resolve) => {
    https.get(`https://cdn.jsdelivr.net/npm/leaflet@${pkg.version}/dist/${path}`, (res) => {
      ssri
        .fromStream(res, { algorithms: ["sha256"] })
        .then((integrity) => resolve(integrity.toString()));
    });
  });
...
docConfig = docConfig
  ...
  .replace(
    ...
    `integrity_hash_global_source: "${integritySrcGlobal}"`,
  )
  .replace(
    /integrity_hash_global_uglified:.*/,
    `integrity_hash_global_uglified: "${integrityUglifiedGlobal}"`,
  )
