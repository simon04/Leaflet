...
export class LeafletMap extends Evented {
  ...
  _handleGeolocationResponse(pos) {
    ...
    for (const i in pos.coords) {
      // do not use Object.keys here to access getters of GeolocationCoordinates
  ...
  _initEvents(remove) {
    ...
    onOff(
      ...
      "click dblclick pointerdown pointerup " +
        "pointerover pointerout pointermove contextmenu keypress keydown keyup",
  ...
  _fireDOMEvent(e, type, canvasTargets) {
    ...
    for (const t of targets) {
      ...
      if (
        ...
        (t.options.bubblingPointerEvents === false && LeafletMap._pointerEvents.includes(type))
      ) {
        return;
      }
    }
